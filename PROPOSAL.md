...
| 2 | Enumerations in the schema | `allowedValues` exists only inside examples. Nothing validates an example, which is why several have drifted. As a per-path `enum` it would be generated, checked and usable by code. Demonstrated on two paths rather than all 261. |
